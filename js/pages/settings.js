...
import { authState, signInWithGoogle, signInWithEmail, registerWithEmail, signOutUser } from '../firebase.js';
...
function accountBodyHtml() {
  ...
  return `
    <button class="btn" id="google-signin" style="margin-bottom:10px;">${icons.cloud} Entrar com Google</button>
    <div style="font-size:12px;color:var(--ink-soft);opacity:.7;margin-bottom:6px;">ou com e-mail</div>
    <div class="field-row">
      <div class="field"><input class="input" id="email" placeholder="E-mail" /></div>
      <div class="field"><input class="input" id="password" type="password" placeholder="Senha" /></div>
    </div>
    <div style="display:flex;gap:8px;">
      <button class="btn ghost" id="email-signin">Entrar</button>
      <button class="btn ghost" id="email-register">Criar conta</button>
    </div>
  `;
}
...
function wireAccount(page) {
  ...
  page.querySelector('#email-signin')?.addEventListener('click', async () => {
    const email = page.querySelector('#email').value.trim();
    const password = page.querySelector('#password').value;
    if (!email || !password) return;
    try { await signInWithEmail(email, password); toast('Login feito.'); }
    catch { toast('E-mail ou senha incorretos.'); }
  });
  page.querySelector('#email-register')?.addEventListener('click', async () => {
    const email = page.querySelector('#email').value.trim();
    const password = page.querySelector('#password').value;
    if (!email || !password) return;
    try { await registerWithEmail(email, password); toast('Conta criada.'); }
    catch { toast('Não foi possível criar a conta.'); }
  });
