...
function rowHtml(h, today) {
  ...
  return `
    <div class="item-row">
      <button class="checkbox ${done ? 'done' : ''}" data-toggle="${h.id}" style="${done ? `background:${color};border-color:${color};` : `border-color:${color};`}">${icons.check}</button>
      <div style="flex:1;">
        ...
        <div style="display:flex;gap:3px;margin-top:6px;">
          ${last7.map((on) => `<span style="width:16px;height:16px;border-radius:5px;background:${on ? color : 'var(--cocoa-50)'};display:inline-block;"></span>`).join('')}
        </div>
      </div>
  ...
  openModal({
    ...
    onMount: (body, close) => {
      ...
      body.querySelector('#save').addEventListener('click', () => {
        ...
        if (!name) return;
        if (existing) store.updateHabit(existing.id, { name, color });
        else store.addHabit({ name, color });
