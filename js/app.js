...
const NAV = [
  ...
  { to: '#/rotinas', label: 'Rotinas', icon: 'layers', page: renderRoutines },
  { to: '#/areas', label: 'Espaços', icon: 'tag', page: renderAreas },
...
function applyTheme() {
  ...
  // Cor padrão (modo claro) usa o tom claro. No modo escuro, --accent (fundo
  // de botões, com texto claro por cima) fica um degrau mais rico, e
  // --accent-strong (usado como TEXTO sobre fundo escuro) precisa ficar mais
  // claro, não mais escuro — senão o texto some contra o fundo escuro.
  ...
  root.style.setProperty('--accent-strong', isDark ? (light[accent] || light.cocoa) : (lightStrong[accent] || lightStrong.cocoa));
}

function currentRoute() {
  // Sem hash definido (app recém-aberto), a tela inicial é a Agenda — assim
  // ela funciona como uma central única, no estilo Google Agenda.
