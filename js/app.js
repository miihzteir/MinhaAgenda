...
import { renderAcademic } from './pages/academic.js';
...
const NAV = [
  ...
  { to: '#/faculdade', label: 'Faculdade', icon: 'academic', page: renderAcademic },
      ...
      <div class="main-wrap" style="flex:1;min-width:0;">
        <header class="topbar" style="padding:14px 20px 0;">
          <div class="mobile-brand">
            <div class="brand-mark" style="width:30px;height:30px;"><span style="width:17px;height:17px;display:inline-block;">${icons.book}</span></div>
            <span class="brand-name" style="font-size:15px;">Minha Agenda</span>
          </div>
          <button class="pill-btn" id="btn-add">${icons.plus} Adicionar</button>
          <button class="avatar" id="btn-profile">${(store.get().profile.name || '?').charAt(0).toUpperCase()}</button>
        </header>
        <main class="main" id="page"></main>
      </div>
      <nav class="bottom-nav">
        ${MOBILE_PRIMARY.slice(0, 2).map((to) => navItemHtml(NAV.find((n) => n.to === to), to === active.to)).join('')}
        <button class="fab" id="btn-fab">${icons.plus}</button>
        ${MOBILE_PRIMARY.slice(2).map((to) => navItemHtml(NAV.find((n) => n.to === to), to === active.to)).join('')}
        <a href="#/mais" class="nav-item ${route === '#/mais' ? 'active' : ''}">${icons.moreH}Mais</a>
      </nav>
    </div>
  `;
