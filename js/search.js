...
function collect(query) {
  ...
  const groups = [
    ...
    { label: 'Metas', icon: 'target', to: '#/metas', items: d.goals.filter((g) => normalize(g.title).includes(q)).map((g) => ({ title: g.title, meta: `${g.progress}%` })) },
    { label: 'Espaços', icon: 'tag', to: '#/areas', items: d.areas.filter((a) => normalize(a.name).includes(q)).map((a) => ({ title: a.name, meta: '' })) },
