...
export const store = {
  ...
  addTask(task) {
    data.tasks.push({
      ...
      eventId: null, areaId: null, categoryId: null, dueDate: null, plannedDate: null, time: null,
  ...
  // Reordena os espaços (arrastar e soltar tipo Trello) — recebe a nova
  // ordem de ids e reconstrói o array na mesma ordem.
  reorderAreas(ids) {
    const byId = new Map(data.areas.map((a) => [a.id, a]));
    const reordered = ids.map((id) => byId.get(id)).filter(Boolean);
    const missing = data.areas.filter((a) => !ids.includes(a.id));
    data.areas = [...reordered, ...missing];
    save();
  },
  deleteArea(id) {
    ...
    data.goals = data.goals.map((g) => (g.areaId === id ? { ...g, areaId: null } : g));
    save();
  },
  ...
  addHabit(h) {
    data.habits.push({
      ...
      type: 'yesno', goal: null, unit: '', preferredTime: null, categoryId: null,
  ...
  // Categoria é usada por eventos, tarefas, hábitos e metas — ao excluir,
  // tudo que estava nela passa pra primeira categoria que sobrou (eventos
  // sempre precisam de uma; tarefas/hábitos/metas/espaços podem ficar sem).
  deleteCategory(id) {
    ...
    data.tasks = data.tasks.map((t) => (t.categoryId === id ? { ...t, categoryId: null } : t));
    data.habits = data.habits.map((h) => (h.categoryId === id ? { ...h, categoryId: null } : h));
    data.goals = data.goals.map((g) => (g.categoryId === id ? { ...g, categoryId: null } : g));
    data.areas = data.areas.map((a) => (a.categoryId === id ? { ...a, categoryId: null } : a));
    save();
  },

  addGoal(g) {
    data.goals.push({
      id: uid(), title: '', description: '', areaId: null, targetDate: null, categoryId: null,
      progress: 0, done: false, createdAt: Date.now(), ...g
    });
    save();
  },
  updateGoal(id, patch) {
    data.goals = data.goals.map((g) => {
      if (g.id !== id) return g;
      const next = { ...g, ...patch };
      if (patch.progress != null) next.done = patch.progress >= 100;
      if (patch.done === true) next.progress = 100;
      return next;
    });
    save();
  },
