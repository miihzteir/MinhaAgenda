...
function draw(page) {
  ...
  page.innerHTML = `
    ...
    <div class="card">
      <div class="card-head"><span class="card-title">Categorias</span><button class="mini-btn" id="add-cat">${icons.plus} Nova</button></div>
      <div class="card-body">
        <p style="font-size:11.5px;color:var(--ink-soft);opacity:.75;margin:0 0 10px;">Usadas em eventos, tarefas, hábitos e espaços — cada uma com uma cor padrão que pode ser trocada em cada item.</p>
        ${d.categories.map((c) => `
          <div class="item-row">
  ...
  page.querySelectorAll('[data-cat-del]').forEach((b) => b.addEventListener('click', () => {
    const inUse = d.events.some((e) => e.category === b.dataset.catDel)
      || d.tasks.some((t) => t.categoryId === b.dataset.catDel)
      || d.habits.some((h) => h.categoryId === b.dataset.catDel)
      || d.areas.some((a) => a.categoryId === b.dataset.catDel);
    if (confirm(inUse ? 'Essa categoria está em uso. Excluir mesmo assim? Os itens ficam sem categoria (eventos passam pra outra).' : 'Excluir essa categoria?')) {
