...
// Reaproveita a mesma lógica de "tarefas de hoje" e o modal de planejar o
// dia que já existem no Início — evita ter dois jeitos diferentes de decidir
// o que é "de hoje" e duplicar o seletor de prioridades.
import { todayTasksFor, openPlanDayModal } from './home.js';
import { getCategories, catOf, catColor } from '../categories.js';
...
function drawDay(body, d, today) {
  ...
  const todayTasks = isToday ? todayTasksFor(d, today) : [];
  const priorityTasks = todayTasks.filter((t) => t.priorityPick && !t.done).slice(0, 3);
  ...
  body.innerHTML = `
    <div class="day-view-wrap">
      ...
      ${isToday ? `
      <div class="card">
        <div class="card-head"><span class="card-title">${icons.star} Foco de hoje</span><button class="mini-btn" id="plan-day-agenda">${icons.edit} Planejar</button></div>
        <div class="card-body">
          ${priorityTasks.length
            ? priorityTasks.map(taskMiniRowHtml).join('')
            : emptyMini('Nenhuma prioridade escolhida ainda. Toque em "Planejar" e escolha até 3 tarefas pra hoje.')}
        </div>
      </div>` : ''}
  ...
  if (isToday) {
    body.querySelector('#plan-day-agenda').addEventListener('click', () => openPlanDayModal(todayTasks, today));
  }
      ...
      <div class="card calendar-card"><div class="card-body">
        ...
        <div class="week-grid">
          ...
          ${cells.map((c) => {
            ...
            const hasTask = c.dateISO ? d.tasks.some((t) => (t.plannedDate || t.dueDate) === c.dateISO && !t.done) : false;
            const cls = ['day'];
            ...
            return `<button class="${cls.join(' ')}" ${c.dateISO ? `data-day="${c.dateISO}"` : 'disabled'}>${c.day}<span class="day-dots">${evs.length ? `<span class="dot"${dotStyle}></span>` : ''}${hasTask ? `<span class="dot task-dot"></span>` : ''}</span></button>`;
          }).join('')}
        </div>
      </div></div>
  ...
  body.innerHTML = `
    ...
    <div class="week-strip">
      ${days.map((day) => {
        ...
        const dayTasks = dayTasksFor(day, d, eventIds);
        const dt = new Date(day + 'T00:00:00');
        const parts = [];
        if (evs.length) parts.push(`${evs.length} evento${evs.length > 1 ? 's' : ''}`);
        if (dayTasks.length) parts.push(`${dayTasks.length} tarefa${dayTasks.length > 1 ? 's' : ''}`);
        return `
          <div class="week-strip-day">
            <div class="week-strip-head ${day === today ? 'today' : ''}" data-expand="${day}">
              ...
              <span class="count">${parts.length ? parts.join(' · ') : 'livre'}</span>
            </div>
            <div class="week-strip-body">
              ${evs.length ? evs.map((e) => eventRowHtml(e, d)).join('') : ''}
              ${dayTasks.length ? dayTasks.map(taskMiniRowHtml).join('') : ''}
              ${!evs.length && !dayTasks.length ? `<div style="font-size:11.5px;color:var(--ink-soft);opacity:.6;padding:4px 6px 8px;">Nada por aqui.</div>` : ''}
            </div>
          </div>`;
      }).join('')}
    </div>
